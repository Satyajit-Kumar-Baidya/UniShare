import { MARKETPLACE_ITEMS, SUBSCRIPTION_GROUPS, REVIEWS, MOCK_USERS, VERIFICATION_REQUESTS } from '../data/mock';
...
export async function getMockUserByEmail(email: string): Promise<MockUser | undefined> {
  if (!USE_MOCK) {
    return apiClient<MockUser>(`/users/by-email/?email=${encodeURIComponent(email)}`);
  }

  await wait();
  const normalized = normalizeEmail(email);
  return MOCK_USERS.find(
    (user) => normalizeEmail(user.email) === normalized || normalizeEmail(user.uiuEmail ?? '') === normalized
  );
}
...
export async function submitVerificationRequest(input: VerificationSubmissionInput): Promise<{
  user: MockUser;
  request: VerificationRequest;
}> {
  if (!USE_MOCK) {
    return apiClient<{ user: MockUser; request: VerificationRequest }>('/verifications/submit/', {
      data: input,
    });
  }
  ...
  const existingIndex = MOCK_USERS.findIndex((user) => normalizeEmail(user.email) === normalizedEmail);
  const existingUser = existingIndex >= 0 ? MOCK_USERS[existingIndex] : undefined;
  ...
  const nextUser: MockUser = {
    ...
    joinedDate:
      existingUser?.joinedDate ?? new Date().toLocaleDateString('en-US', { month: 'long', year: 'numeric' }),
  };
  ...
  return { user: existingIndex >= 0 ? MOCK_USERS[existingIndex] : MOCK_USERS[0], request };
}
...
export async function approveVerificationRequest(requestId: string, adminNote?: string): Promise<{
  request?: VerificationRequest;
  user?: MockUser;
}> {
  if (!USE_MOCK) {
    return apiClient<{ request?: VerificationRequest; user?: MockUser }>(`/verifications/${requestId}/approve/`, {
      data: { adminNote },
    });
  }
...
export async function rejectVerificationRequest(requestId: string, adminNote?: string): Promise<{
  request?: VerificationRequest;
  user?: MockUser;
}> {
  if (!USE_MOCK) {
    return apiClient<{ request?: VerificationRequest; user?: MockUser }>(`/verifications/${requestId}/reject/`, {
      data: { adminNote },
    });
  }
  ...
  request.reviewedAt = new Date().toISOString();
  request.adminNote = adminNote || 'Please review and resubmit your verification.';
